...
class ProductProvider extends ChangeNotifier {
  ...
  getAllProducts() {
    ProductRepository.getAllProducts().listen((snapshot) {
      productList = List.generate(snapshot.docs.length,
              (index) => ProductModel.fromFirestore(snapshot.docs[index]));
      notifyListeners();
    });
  }
  ...
  getAllPurchases() {
    ProductRepository.getAllPurchases().listen((snapshot) {
      purchaseList = List.generate(snapshot.docs.length,
              (index) => PurchaseModel.fromFirestore(snapshot.docs[index]));
      notifyListeners();
    });
  }
  ...
  getAllReviewsByProduct(String productId) {
    ProductRepository.getAllReviewsByProduct(productId).listen((snapshot) {
      reviewList = List.generate(snapshot.docs.length,
              (index) => ReviewModel.fromFirestore(snapshot.docs[index]));
      notifyListeners();
    });
  }

  Future<void> addReview(ReviewModel reviewModel) {
    return ProductRepository.addReview(reviewModel);
  }
}
